...
export default function Player({ url, media, onProgress, startAt = 0, durationHint = 0, onEnded, onNext }) {
  ...
  return (
    ...
    >
          ...
          {audioTracks.length > 1 && (
            <div className="relative">
              <button
                ...
                className="bg-black/60 hover:bg-black/70 border border-white/20 text-white rounded-lg w-10 h-9 text-xs font-bold"
                title="Audio"
              >
                A
              </button>
              {audioMenuOpen && (
                ...
                >
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm ${audioSelected === null ? "bg-white/10 text-white" : "text-white/90 hover:bg-white/10"}`}
                  >
                    Auto
                  </button>
                  {audioTracks.map((t) => {
                    ...
                    const active = audioSelected === t.stream_index;
                    return (
                      <button
                        key={t.stream_index}
                        onClick={() => {
                          audioSeekRef.current = absTimeRef.current;
                          setAudioSelected(t.stream_index);
                          setAudioLabel(t.language || t.title || "Audio");
                          setAudioMenuOpen(false);
                        }}
                        className={`w-full text-left px-3 py-2 text-sm ${active ? "bg-white/10 text-white" : "text-white/90 hover:bg-white/10"}`}
                      >
                        {label}
                      </button>
                    );
              ...
              {subMenuOpen && (
                ...
                >
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm ${subtitleSelected === null ? "bg-white/10 text-white" : "text-white/90 hover:bg-white/10"}`}
                  >
                    Uit
                  </button>
                  {subtitleTracks.map((t) => {
                    ...
                    const active = subtitleSelected === t.stream_index;
                    return (
                      <button
                        key={t.stream_index}
                        onClick={() => {
                          setSubtitleSelected(t.stream_index);
      ...
      {!effectiveBuffering && !error && effectivePlaying && flashIcon && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
